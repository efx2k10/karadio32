...
typedef enum {KEY_UP,KEY_LEFT,KEY_OK,KEY_RIGHT,KEY_DOWN,
		...
		KEY_STAR,KEY_DIESE,KEY_INFO, KEY_ONOFF,KEY_MAX} customKey_t;
...
gpio_pad_select_gpio(PIN_IN_AUDIO);
gpio_set_direction(PIN_IN_AUDIO, GPIO_MODE_OUTPUT);
	...
	if (inaudio == 0 ) {
		...
		stopStation();
		isModeChanged = 0;
		gpio_set_level(PIN_IN_AUDIO, inaudio);
	}
	else {
		inaudio = 0;
		isModeChanged = 1;
...
void customKeyInit()
{
	customKey_t index;
	nvs_handle handle;
	const char *klab[] = {"K_UP","K_LEFT","K_OK","K_RIGHT","K_DOWN","K_0","K_1","K_2","K_3","K_4","K_5","K_6","K_7","K_8","K_9","K_STAR","K_DIESE","K_INFO","K_ONOFF"};
